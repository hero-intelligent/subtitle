...
srt_data = parse_srt_file("c:\\Users\\hero\\Downloads\\Extracted_星光闪耀的少年_jp_ptEP10_ 总决赛现场直播 九位少年成团出道_简体中文_语料1.srt")

srt_events = srt_data["Events"]

translated_event = []
untranslated_event = []
for event in srt_events:
    if "#" in event["Text"]:
        untranslated_event.append(event)
    else:
        translated_event.append(event)


translated_data = copy.deepcopy(srt_data)
translated_data["Events"] = translated_event

untranslated_data = copy.deepcopy(srt_data)
untranslated_event["Events"] = untranslated_event
